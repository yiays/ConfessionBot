...
class Prefix(commands.Cog):
  ...
  def check_prefix(self, bot, message:disnake.Message):
    if isinstance(message.channel, disnake.TextChannel):
      if (
        str(message.channel.guild.id) in self.bot.config['prefix'] and
        len(self.bot.config['prefix'][str(message.channel.guild.id)])
      ):
        if message.content.lower()\
          .startswith(self.bot.config['prefix'][str(message.channel.guild.id)].lower()):
  ...
  async def prefix_get(self, ctx:commands.Context, guild:int = 0):
    await ctx.reply(self.bot.babel(
      ...
      'prefix',
      'get',
      prefix=self.bot.config.get('prefix', str(guild if guild else ctx.guild.id), fallback='*unset*')
    ))
